...
def create_df():
    #create one dataframe from the csv files, drop the rows with header data from the conversion process. update header row.
    df = pd.DataFrame()
    ...
    df.columns = ['voter', 'registration_date', 'party', 'precinct', 'ward', 'smd', 'election_0415', 'election_1114', 'election_0714', 'election_0414', 'election_0413', 'election_1112']
...
def compile_csv():
    ...
    voters['last_name'], voters['first_name'], voters['initial'],  = parse_name(voters.voter)
